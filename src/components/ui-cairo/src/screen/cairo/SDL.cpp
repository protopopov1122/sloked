...
namespace sloked {
    ...
    SlokedSDLCairoSurface::SlokedSDLCairoSurface(int width, int height)
        ...
        this->cairoSurface = Cairo::ImageSurface::create(
            ...
            Cairo::Format::FORMAT_RGB24, this->sdlSurface.GetSurface()->w,
    ...
    Cairo::RefPtr<Cairo::Surface> SlokedSDLCairoSurface::GetCairoSurface()
        const {
    ...
    void SlokedSDLCairoSurface::Resize(int width, int height) {
        ...
        auto newSurface = Cairo::ImageSurface::create(
            ...
            Cairo::Format::FORMAT_RGB24, newSdlSurface.GetSurface()->w,
            newSdlSurface.GetSurface()->h, newSdlSurface.GetSurface()->pitch);
    ...
    SlokedSDLTexture SlokedSDLCairoSurface::MakeTexture(
        SDL_Renderer *renderer) {
        return SlokedSDLTexture(renderer, this->sdlSurface);
    }

    SlokedSDLCairoWindow::SlokedSDLCairoWindow(SlokedScreenManager &screenMgr,
                                               SlokedGraphicsDimensions dim,
                                               const std::string &title)
        : screenMgr(screenMgr), sdlWindow({dim.x, dim.y}, title),
          sdlRenderer(sdlWindow), rootSurface{dim.x, dim.y}, root{nullptr},
          resize_request{false}, title{sdlWindow.Title()} {
    ...
    std::shared_ptr<SlokedCairoScreenComponent> SlokedSDLCairoWindow::GetRoot()
        const {
        return this->root;
    }

    void SlokedSDLCairoWindow::SetRoot(
        std::shared_ptr<SlokedCairoScreenComponent> root) {
        this->root = root;
        this->root->SetTarget(
            this->rootSurface.GetCairoSurface(),
            {this->rootSurface.GetWidth(), this->rootSurface.GetHeight()});
    }
    ...
    void SlokedSDLCairoWindow::Render() {
        ...
        if (this->resize_request) {
            ...
            this->rootSurface.Resize(this->new_size.first,
                                     this->new_size.second);
            if (this->root) {
                this->root->SetTarget(
                    this->rootSurface.GetCairoSurface(),
                    {this->new_size.first, this->new_size.second});
            }
        }
        if (this->sdlWindow.IsOpen() && this->root &&
            this->root->CheckUpdates()) {
            SDL_RenderClear(this->sdlRenderer.GetRenderer());
            if (this->root) {
                auto texture = this->rootSurface.MakeTexture(
                    this->sdlRenderer.GetRenderer());
                SDL_RenderCopy(this->sdlRenderer.GetRenderer(),
                               texture.GetTexture(), nullptr, nullptr);
            }
            ...
            while (eventSource.HasEvents()) {
                ...
                switch (evt.type) {
                    ...
                    case SDL_TEXTEDITING:
                        events.emplace_back(SlokedKeyboardInput{
                            std::string{evt.edit.text}, false});
                        break;

                    case SDL_TEXTINPUT:
                        events.emplace_back(SlokedKeyboardInput{
                            std::string{evt.text.text}, false});
                        break;

                    case SDL_WINDOWEVENT:
                        switch (evt.window.event) {
                            case SDL_WINDOWEVENT_RESIZED: {
                                this->new_size = std::make_pair(
