...
namespace sloked {
    ...
    std::map<int64_t, std::function<void()>> SlokedTerminalResizeListener::listeners{};

    std::function<void()> SlokedTerminalResizeListener::Bind(std::function<void()> listener) {
        ...
        SlokedTerminalResizeListener::listeners.emplace(id, std::move(listener));
        return [id] {
            std::unique_lock lock(SlokedTerminalResizeListener::mtx);
    ...
    void SlokedTerminalResizeListener::Trigger(int) {
        std::unique_lock lock(SlokedTerminalResizeListener::mtx);
        for (auto it = SlokedTerminalResizeListener::listeners.begin(); it != SlokedTerminalResizeListener::listeners.end();) {
            auto current = it++;
            lock.unlock();
            current->second();
            lock.lock();
...
namespace sloked {
    ...
    std::function<void()> SlokedTerminalResizeListener::Bind(std::function<void()>) {
        return []{};
    }
}
#endif