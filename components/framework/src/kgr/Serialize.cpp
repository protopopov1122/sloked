...
namespace sloked {
    ...
    class KgrBinarySerializer::ByteIter {
     public:
        ByteIter(std::vector<SlokedBase64::Byte>::const_iterator begin, std::vector<SlokedBase64::Byte>::const_iterator end)
    ...
    void SerializeScalar(T value, I inserter) {
        ...
        if constexpr (Bytes > 0) {
            ...
            inserter = byte;
            (SerializeScalar<uint64_t, I, (Bytes - 1)>)(uvalue.raw >> 8, ++inserter);
        }
    }
    ...
    void KgrBinarySerializer::SerializeValue(KgrSerializer::Blob &output, const KgrValue &value) const {
        switch (value.GetType()) {
            case KgrValueType::Null:
            ...
            case KgrValueType::Boolean:
                output.push_back(static_cast<int8_t>(value.AsBoolean() ? Tag::BooleanTrue : Tag::BooleanFalse));
                break;
            ...
            case KgrValueType::Object: {
                ...
                for (const auto &el : object) {
                    if (SlokedLocale::SystemEncoding() != this->encoding) {
                        ...
                        SerializeScalar(static_cast<uint32_t>(el.first.size()), std::back_inserter(output));
                        output.insert(output.end(), el.first.begin(), el.first.end());
                    }
    ...
    T DeserializeScalar(KgrBinarySerializer::ByteIter &iter, uint64_t value = 0) {
        if constexpr (Bytes > 0) {
            return DeserializeScalar<T, Bytes - 1>(iter, value | (static_cast<uint64_t>(iter.Next()) << ((sizeof(T) - Bytes) * 8)));
        } else {
            union {
                uint64_t raw;
                T value;
