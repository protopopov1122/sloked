...
namespace sloked {
    ...
    std::unique_ptr<TextBlock> TextView::Open(
        std::string_view str, const NewLine &newline,
        ...
        newline.Iterate(str, [&](std::size_t i, std::size_t width) {
            ...
            if (length < MAX_CHUNK) {
                chunk_lines[chunk_lines.size()] = std::make_pair(
                    last_line_offset - chunk_offset, i - last_line_offset);
                last_line_offset = i + width;
            } else {
                auto proxyBlock = std::make_unique<TextBlockHandle>(
                    str.substr(chunk_offset,
                               last_line_offset > chunk_offset
                                   ? last_line_offset - chunk_offset - width
                                   : 0),
                    std::move(chunk_lines), blockFactory);
                auto region = std::make_unique<TextRegion>(
                    newline, std::move(proxyBlock));
                if (content) {
        ...
        chunk_lines[chunk_lines.size()] = std::make_pair(
            last_line_offset - chunk_offset, str.size() - last_line_offset);

        if (chunk_offset != str.size()) {
            auto proxyBlock = std::make_unique<TextBlockHandle>(
                str.substr(chunk_offset), std::move(chunk_lines), blockFactory);
            auto region =
                std::make_unique<TextRegion>(newline, std::move(proxyBlock));
            if (content) {
                content->AppendRegion(std::move(region));
            } else {
                content = std::move(region);
