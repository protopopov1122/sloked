...
namespace sloked {
    ...
    TextRegion::TextRegion(const NewLine &newline,
                           std::unique_ptr<TextBlock> content)
        : AVLNode(nullptr, nullptr), newline(newline),
          content(std::move(content)), height(0), last_line(0) {
        this->UpdateStats();
    }
    ...
    std::string_view TextRegion::GetLine(std::size_t line) const {
        const std::size_t begin_length =
            this->begin ? this->begin->GetLastLine() + 1 : 0;
        const std::size_t self_length =
            this->content ? this->content->GetLastLine() + 1 : 0;
        if (this->begin && line <= this->begin->GetLastLine()) {
            return this->begin->GetLine(line);
        } else if (this->content &&
                   line <= begin_length + this->content->GetLastLine()) {
            return this->content->GetLine(line - begin_length);
        } else if (this->end && line <= begin_length + self_length +
                                            this->end->GetLastLine()) {
    ...
    void TextRegion::Visit(std::size_t start, std::size_t count,
        ...
        if (count && this->begin && start <= this->begin->GetLastLine()) {
            const std::size_t read =
                std::min(count, this->begin->GetLastLine() - start + 1);
            this->begin->Visit(start, read, visitor);
            count -= read;
            start += read;
        }
        if (count && this->content &&
            start <= begin_length + this->content->GetLastLine()) {
            const std::size_t node_start = start - begin_length;
            const std::size_t read =
                std::min(count, this->content->GetLastLine() - node_start + 1);
            this->content->Visit(node_start, read, visitor);
            count -= read;
            start += read;
        }
        if (count && this->end &&
            start <= begin_length + self_length + this->end->GetLastLine()) {
            const std::size_t node_start = start - begin_length - self_length;
            const std::size_t read =
                std::min(count, this->end->GetLastLine() - node_start + 1);
