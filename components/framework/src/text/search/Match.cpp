...
namespace sloked {
    ...
    SlokedTextMatcherBase::SlokedTextMatcherBase(const TextBlockView &text,
                                                 const Encoding &encoding)
        : text(text), conv(encoding, SlokedLocale::SystemEncoding()),
          current_line(0) {}

    const std::vector<SlokedTextMatcherBase::Result>
        &SlokedTextMatcherBase::GetResults() const {
        return this->occurences;
    }

    void SlokedTextMatcherBase::Rewind(const TextPosition &position) {
        this->occurences.erase(
            std::remove_if(this->occurences.begin(), this->occurences.end(),
                           [&](const auto &match) {
    ...
    SlokedTextRegexMatcher::SlokedTextRegexMatcher(const TextBlockView &text,
                                                   const Encoding &encoding,
                                                   bool enable_groups)
    ...
    void SlokedTextRegexMatcher::Match(const std::string &query, Flags flags) {
        ...
        this->regexp =
            std::regex(query, static_cast<std::regex::flag_type>(regex_prms));
        this->Search();
    }
    ...
    void SlokedTextRegexMatcher::Search() {
        this->text.Visit(
            this->current_line,
            this->text.GetLastLine() - this->current_line + 1,
            [&](const auto lineView) {
                std::smatch match;
                ...
                while (std::regex_search(line, match, this->regexp)) {
                    TextPosition pos{
                        ...
                        static_cast<TextPosition::Column>(
                            this->conv.GetDestination()
                                .GetCodepointByOffset(line, match.position())
                                .value_or(0) +
                            offset)};
                    TextPosition::Column length =
                        this->conv.GetDestination().CodepointCount(match.str());
                    std::vector<std::string> groups;
                    if (this->enable_groups) {
                        for (std::size_t i = 0; i < match.size(); i++) {
                            groups.push_back(
                                this->conv.ReverseConvert(match.str(i)));
                        }
                    }
                    this->occurences.push_back(
                        Result{pos, length,
                               this->conv.ReverseConvert(match.str()), groups});
    ...
    SlokedTextPlainMatcher::SlokedTextPlainMatcher(const TextBlockView &text,
                                                   const Encoding &encoding)
    ...
    void SlokedTextPlainMatcher::Match(const std::string &query, Flags flags) {
        ...
        this->conv.GetDestination().IterateCodepoints(
            query, [&](auto start, auto length, auto chr) {
                // Escaping raw string
                switch (chr) {
                    case U'[':
                    case U'\\':
        ...
        this->SlokedTextRegexMatcher::Match(
            this->conv.GetDestination().Encode(escapedQuery), flags);
    }
}  // namespace sloked